      ...
      if (existingListing) {
        ...
        if (statusChanged) {
          await db.database.from('listings').update({
            ...
            bedrooms: p.bedrooms ?? existingListing.bedrooms,
            bathrooms: p.bathrooms ?? existingListing.bathrooms,
            sqft_or_acreage: p.sqft_or_acreage ?? existingListing.sqft_or_acreage,
        ...
        await db.database.from('listings').insert([{
          ...
          sqft_or_acreage: p.sqft_or_acreage,
          email_received_at: p.email_received_at.toISOString(),
          build_status: 'ready',
        }])
        created++
